...
const Meeting = () => {
  ...
  const userTimezone = localStorage.getItem('timezone') || moment.tz.guess();
  ...
  const getAvailableTimes = () => {
    ...
    if (selectedDay) {
      const eventDuration = parseInt(event.eventduration, 10); // Convert event duration to a number (in minutes)

      selectedDay.schedule_times.forEach((time) => {
        const startMoment = moment.tz(time.start, 'hh:mm A', 'UTC').tz(userTimezone)
        const endMoment = moment.tz(time.end, 'hh:mm A', 'UTC').tz(userTimezone)
  
        while (endMoment.isAfter(startMoment)) {
          const endTimeClone = moment.tz(startMoment, userTimezone); // Clone the startMoment for the end time option
          endTimeClone.add(eventDuration, 'minutes'); // Add event duration to create the end time option

          availableTimes.push({
            value: {
              start: startMoment.format('hh:mm A'),
              end: endTimeClone.format('hh:mm A'),
            },
            label: `${startMoment.format('hh:mm A')} - ${endTimeClone.format('hh:mm A')}`,
          });

          startMoment.add(eventDuration, 'minutes'); // Move to the next time option
        }
      });
    }
                        ...
                        <Row>
                          <Col sm={9}>
                            <Row className="addscroll">
                              ...
                              <TimezoneSelector userId={user?.userId} onUpdate={() => window.location.href = `/meeting/${link}`} />
